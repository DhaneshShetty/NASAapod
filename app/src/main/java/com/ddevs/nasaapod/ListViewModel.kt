...
class ListViewModel: ViewModel() {
    ...
    private fun getTodayApod(){
        viewModelScope.launch {
            try {
                _todayApod.value = ApodAPI.service.getTodayApod("sdV5nVTPFuigmh7gySTWgg4IiLCRl95gjbdzGiGZ")
            }
            catch(e:Exception){
    ...
    private fun getListApod(){
        viewModelScope.launch {
            try {
                _listApod.value = ApodAPI.service.getList("sdV5nVTPFuigmh7gySTWgg4IiLCRl95gjbdzGiGZ")
            }
            catch(e:Exception){
                status.value="ERROR Loading Data"
            }
